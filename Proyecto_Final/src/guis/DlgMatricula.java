...
public class DlgMatricula extends JDialog implements ActionListener, KeyListener, MouseListener {
	...
	protected void actionPerformedBtnBuscar(ActionEvent arg0) {
		try{
			...
			error("Ingrese NÚMERO válido", txtNumero);
		}
	}
			...
			try{
				...
				if(btnAdicionar.isEnabled() == false){
					...
					if(a!=null && c!=null){
						...
						error("El código del alumno o curso no ha sido registrado", txtCodigoAlumno);
						limpiar();
						btnAdicionar.setEnabled(false);
					}	
				}
				...
				error("Ingrese el CÓDIGO del curso",txtCodigoCurso);
			}
		}
		catch(Exception e){
			error("Ingrese el CÓDIGO del alumno",txtCodigoAlumno);
		}
	...
	void ajustarAnchoColumnas(){
		...
		tcm.getColumn(0).setPreferredWidth(anchoColumna(4));  // numMatrícula
